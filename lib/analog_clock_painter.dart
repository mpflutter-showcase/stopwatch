...
class AnalogClockPainter extends CustomPainter {
  static final _textPainterCache = <String, MPTextPainter>{};
  ...
  Future paintAsync(Canvas canvas, Size size) async {
    ...
    if (showNumbers) {
      await _drawIndicators(canvas, size, scaleFactor, showAllNumbers);
    }

    if (showDigitalClock)
      await _paintDigitalClock(canvas, size, scaleFactor, useMilitaryTime);
  ...
  Future _drawIndicators(
      Canvas canvas, Size size, double scaleFactor, bool showAllNumbers) async {
    ...
    for (var h = 1; h <= 12; h++) {
      ...
      MPTextPainter tp;
      if (_textPainterCache[h.toString()] != null) {
